    ...
    <div class="flex-1 p-4 md:p-8">
        <div class="flex flex-wrap justify-between items-center mb-6">
            ...
            <button onclick="document.getElementById('offlineModal').classList.add('active')" 
                    class="btn-purple">
                <i class="fas fa-plus mr-1"></i> Tambah Offline
            </button>
        </div>
                ...
                <table class="w-full min-w-[1000px]">
                    ...
                    <tbody>
                        ...
                        <tr class="border-t hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm">#{{ $loop->iteration + ($offlineBookings->currentPage() - 1) * $offlineBookings->perPage() }}</td>
                            <td class="px-4 py-3 text-sm">
                                <div class="font-medium">{{ $offline->customer_name }}</div>
                                <div class="text-xs" style="color: var(--text-card);">{{ $offline->customer_phone }}</div>
                            </td>
                            <td class="px-4 py-3 text-sm whitespace-nowrap">{{ \Carbon\Carbon::parse($offline->check_in)->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-sm whitespace-nowrap">{{ \Carbon\Carbon::parse($offline->check_out)->format('d/m/Y') }}</td>
                            <td class="px-4 py-3 text-sm">
                                @php
                            ...
                            <td class="px-4 py-3 text-sm font-bold whitespace-nowrap">Rp {{ number_format($offline->total_price, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-sm">
                            ...
                            <td class="px-4 py-3 text-sm">
                                ...
                                <button onclick="deleteOffline('{{ $offline->id }}', '{{ $offline->customer_name }}')" 
...
<div class="modal-overlay" id="offlineModal">
    <div class="modal-content">
        <button class="modal-close" onclick="this.closest('#offlineModal').classList.remove('active')">
            <i class="fas fa-times"></i>
        </button>
        
        <h2 class="text-xl font-bold mb-4">Tambah Booking Offline</h2>
        
        <form method="POST" action="{{ route('admin.offline.store') }}">
            @csrf
            
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1" style="color: var(--text-body);">Nama Customer *</label>
                <input type="text" name="customer_name" required class="form-input">
            </div>

            <div class="mb-3">
                <label class="block text-sm font-medium mb-1" style="color: var(--text-body);">No HP *</label>
                <input type="text" name="customer_phone" required class="form-input">
            </div>

            <div class="mb-3">
                <label class="block text-sm font-medium mb-1" style="color: var(--text-body);">Email</label>
            ...
            <div class="flex gap-3">
                ...
                <button type="button" onclick="this.closest('#offlineModal').classList.remove('active')" 
...
<div class="modal-overlay" id="editModal">
    <div class="modal-content">
        <button class="modal-close" onclick="this.closest('#editModal').classList.remove('active')">
            <i class="fas fa-times"></i>
        </button>
        
        <h2 class="text-xl font-bold mb-4">Edit Booking Offline</h2>
        ...
        <form id="editForm" method="POST">
            ...
            <div class="flex gap-3">
                ...
                <button type="button" onclick="this.closest('#editModal').classList.remove('active')" 
                    class="btn-primary flex-1" style="background-color: var(--border-color); color: var(--text-body);">
                    Batal
                </button>
            </div>
