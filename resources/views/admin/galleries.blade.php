...
<head>
    ...
    <style>
        * { font-family: 'Plus Jakarta Sans', system-ui, sans-serif; margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .dark-mode .btn-blue:hover { background-color: #2563eb; }
        ...
        .dark-mode .btn-delete { color: #f87171; }
        .dark-mode .btn-delete:hover { color: #ef4444; }
    </style>
</head>
    ...
    <div class="flex-1 p-4 md:p-8">
        <div class="flex flex-wrap justify-between items-center mb-6">
            <h1 class="text-2xl md:text-3xl font-bold" style="color: var(--text-body);">
                <i class="fas fa-images mr-2"></i> Gallery Villa
            </h1>
            <button onclick="document.getElementById('galleryModal').classList.add('active')" 
                    class="btn-blue">
            ...
            <div class="gallery-card">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-2 pt-2 border-t" style="border-color: var(--border-color);">
                        <form action="{{ route('admin.galleries.destroy', $gallery->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus foto \"{{ $gallery->title }}\"?')">
                            @csrf
                            @method('DELETE')
        ...
        @else
        <div class="text-center py-12" style="color: var(--text-card);">
            <i class="fas fa-images text-4xl mb-3 block" style="color: var(--text-body);"></i>
            <p class="text-lg">Belum ada foto gallery</p>
            <p class="text-sm">Klik tombol "Tambah Foto" untuk menambahkan</p>
        </div>
        @endif
    </div>
</div>
...
<div class="modal-overlay" id="galleryModal">
    <div class="modal-content">
        <button class="modal-close" onclick="this.closest('#galleryModal').classList.remove('active')">
            <i class="fas fa-times"></i>
        </button>
        
        <h2 class="text-xl font-bold mb-4">Tambah Foto Gallery</h2>
        
        <form method="POST" action="{{ route('admin.galleries.store') }}" enctype="multipart/form-data">
            @csrf
            
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1" style="color: var(--text-body);">Judul *</label>
                <input type="text" name="title" required class="form-input" placeholder="Masukkan judul foto">
            </div>
            
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1" style="color: var(--text-body);">Deskripsi</label>
                <textarea name="description" rows="2" class="form-textarea" placeholder="Deskripsi foto (opsional)"></textarea>
            </div>
            
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1" style="color: var(--text-body);">Foto *</label>
                <input type="file" name="image" accept="image/*" required class="form-file">
                <p class="text-xs mt-1" style="color: var(--text-card);">Format: JPG, PNG, JPEG. Maks: 2MB</p>
            </div>
            
            <div class="flex gap-3">
                <button type="submit" class="btn-blue flex-1">
                    <i class="fas fa-save mr-1"></i> Simpan
                </button>
                <button type="button" onclick="this.closest('#galleryModal').classList.remove('active')" 
